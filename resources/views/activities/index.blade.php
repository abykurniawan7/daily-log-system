        ...
        <div class="flex justify-between items-center">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                    ...
                    @elseif(auth()->user()->role === 'perizinan' || (auth()->user()->role === 'karyawan' && auth()->user()->bagian === 'PKJ'))
                        {{-- PKJ: Semua Aktivitas PKJ --}}
                        {{ __('activities.all_activities_pkj') }}
                    @elseif(auth()->user()->role === 'kabag_pgb')
                        {{ __('activities.all_activities_pgb') }}
                    @else
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-8 gap-4 items-end">
                                    ...
                                    <div>
                                        ...
                                        <select name="user_id" class="w-full text-sm rounded-md border-gray-300 shadow-sm focus:border-[#0F5132] focus:ring-[#0F5132]">
                                            ...
                                            {{-- ✅ REVISED: Include all roles (supervisi, kabag_pgb, perizinan, karyawan) --}}
                                            @php
                                                $allUsers = \App\Models\User::whereIn('role', ['supervisi', 'kabag_pgb', 'perizinan', 'karyawan'])
                                                    ->orderByRaw("FIELD(role, 'supervisi', 'kabag_pgb', 'perizinan', 'karyawan')")
                                                    ->orderBy('name')
                                                    ->get()
                                                    ->groupBy('role');
                                            @endphp
                                            ...
                                            {{-- Supervisi (Kadiv) --}}
                                            @if(isset($allUsers['supervisi']) && $allUsers['supervisi']->count() > 0)
                                                <optgroup label="━━━ Supervisi ━━━">
                                                    @foreach($allUsers['supervisi'] as $user)
                                            ...
                                            {{-- Kabag PGB --}}
                                            @if(isset($allUsers['kabag_pgb']) && $allUsers['kabag_pgb']->count() > 0)
                                                <optgroup label="━━━ Kabag PGB ━━━">
                                                    @foreach($allUsers['kabag_pgb'] as $user)
                                                        <option value="{{ $user->id }}" {{ request('user_id') == $user->id ? 'selected' : '' }}>
                                                            {{ $user->name }} (Kabag - {{ $user->bagian }})
                                                        </option>
                                                    @endforeach
                                                </optgroup>
                                            @endif
                                            ...
                                            {{-- Perizinan (Kabag PKJ) --}}
                                            @if(isset($allUsers['perizinan']) && $allUsers['perizinan']->count() > 0)
                                                <optgroup label="━━━ Kabag PKJ ━━━">
                                                    @foreach($allUsers['perizinan'] as $user)
                                                        <option value="{{ $user->id }}" {{ request('user_id') == $user->id ? 'selected' : '' }}>
                                                            {{ $user->name }} (Kabag - {{ $user->bagian }})
                                                        </option>
                                                    @endforeach
                                            ...
                                            {{-- Karyawan (Staff PGB & PKJ) --}}
                                            @if(isset($allUsers['karyawan']) && $allUsers['karyawan']->count() > 0)
                                                <optgroup label="━━━ Karyawan ━━━">
                                                    @foreach($allUsers['karyawan'] as $user)
                                                        <option value="{{ $user->id }}" {{ request('user_id') == $user->id ? 'selected' : '' }}>
