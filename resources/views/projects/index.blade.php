    <x-app-layout>
        ...
        <x-slot name="header">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                ...
                {{-- ✅ UPDATED: Tambah Project Button - SUPERVISI, KABAG_PGB & PERIZINAN (Kabag PKJ) --}}
                @if(in_array(auth()->user()->role, ['supervisi', 'kabag_pgb', 'perizinan']))
                    ...
                    <div class="p-4 md:p-6 text-gray-900">
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead class="bg-gray-50">
                                        <tr>
                                            ...
                                            {{-- ✅ UPDATED: Kolom Aksi - Semua kecuali Karyawan (Staff PGB) --}}
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        @foreach($projects as $index => $project)
                                            <tr class="hover:bg-[#D1FAE5] transition-colors duration-150 cursor-pointer group" 
                                                ...
                                                <td class="px-6 py-4 text-sm text-gray-900">
                                                    {{-- ✅ Display first PIC (or multiple if needed) --}}
                                                    @if($project->pics->count() > 0)
                                                        {{ $project->pics->first()->name }}
                                                        @if($project->pics->count() > 1)
                                                            <span class="text-xs text-gray-500">(+{{ $project->pics->count() - 1 }})</span>
                                                        @endif
                                                    @else
                                                        <span class="text-gray-400">-</span>
                                                    @endif
                                                ...
                                                {{-- ✅ UPDATED: Kolom Aksi dengan Authorization Logic Baru --}}
                                                @if(auth()->user()->role !== 'karyawan')
                                                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium" onclick="event.stopPropagation()">
                                                        <div class="flex items-center justify-center gap-2">
                                                            @php
                                                                ...
                                                                // ✅ Logic: Hanya PEMBUAT PROJECT yang bisa edit/delete
                                                                if ($user->role === 'supervisi' && $project->user_id === $user->id) {
                                                                    $canEditProject = true;
                                                                }
                                                                elseif ($user->role === 'kabag_pgb' && $project->user_id === $user->id) {
                                                                    $canEditProject = true;
                                                                }
                            ...
                            <div class="lg:hidden space-y-4">
                                @foreach($projects as $project)
                                    <div class="bg-gray-50 rounded-lg p-4 border border-gray-200 hover:border-[#0F5132] hover:bg-[#D1FAE5] hover:shadow-md transition-all duration-150 cursor-pointer"
                                        ...
                                        {{-- ✅ UPDATED: Action Buttons untuk Mobile --}}
                                        @if(auth()->user()->role !== 'karyawan')
                                            @php
                                                $user = auth()->user();
                                                $canEditProject = false;
                        ...
                        @else
                            ...
                            <div class="text-center py-12">
                                ...
                                @if(!request()->hasAny(['search', 'status', 'urgensi', 'division_id']) && in_array(auth()->user()->role, ['supervisi', 'kabag_pgb', 'perizinan']))
                                    <div class="mt-6">
